...
describe Cours do
  ...
  describe ".new" do
    ...
    _it_ "cree un cours avec des prealables" do
      inf2120.prealables.must_equal [:INF1120]
    end

    _it_ "cree un cours inactif" do
  ...
  describe "#<=>" do
    _it_ "retourne 0 par rapport a lui-meme" do
      (inf1120 <=> inf1120).must_equal 0
    end

    _it_ "retourne -1 par rapport a un sigle plus grand" do
      (inf1120 <=> inf2120).must_equal( -1 )
    end

    _it_ "retourne +1 par rapport a un sigle plus petit_" do
  ...
  describe "#to_s -- tests_base" do
    ...
    _it_ "produit_ les bons elements, meme lorsqu'un it_em apparait_ plusieurs" do
      inf1120.to_s( "%S %S %C %T %S" ).must_equal 'INF1120 INF1120 3 Programmation I INF1120'
    end

    _it_ "inclut les diverses chaines qui ne sont pas des formats" do
  ...
  describe "#to_s -- tests_intermediaire" do
    _it_ "traite les justifications et la largeur maximum", :intermediaire do
      inf1120.to_s( "%9S:%-9S:%.9S" ).must_equal '  INF1120:INF1120  :INF1120'
    end

    _it_ "genere une erreur quand une specification de champ non valide est indiquee", :intermediaire do
  ...
  describe "#to_s -- tests_intermediaire" do
    _it_ "traite la specification de separateur de prealables par defaut", :intermediaire do
      mat3143.to_s( "(%P)" ).must_equal '(INF1120:INF2120)'
    end

    _it_ "traite la specification de separateur de prealables explicite", :intermediaire do
      mat3143.to_s( "%P", " " ).must_equal 'INF1120 INF2120'
    end
  end

  describe "#to_s" do
    _it_ "assure que le format n'est pas modifie par une utilisation", :intermediaire do
      format = "%T => %S"
  ...
  describe "#activer/desactiver" do
    _it_ "est actif lorsqu'on le cree" do
      assert inf1120.actif?
    end

    _it_ "devient inactif si on le desactive explicit_ement" do
      inf1120.desactiver

      refute inf1120.actif?
    end

    _it_ "redevient actif si on l'active apres l'avoir desactive" do
      inf1120.desactiver
      inf1120.activer
